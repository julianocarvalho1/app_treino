...
# Inicializa cor no estado se não existir
if 'cor_zebrado' not in st.session_state:
    st.session_state.cor_zebrado = '#262730'

# CSS Dinâmico injetado via f-string
st.markdown(f"""
    <style>
    ...
    /* Cor alternada dinâmica */
    .bg-alt {{
        background-color: {st.session_state.cor_zebrado} !important;
    }}
        ...
        if not df_preview.empty:
            for i, row in df_preview.iterrows():
                ...
                if img_url and isinstance(img_url, str) and len(img_url.strip()) > 5:
                    col1.markdown(f'<a href="{img_url}" target="_blank"><img src="{img_url}" width="60" style="border-radius:8px;"></a>', unsafe_allow_html=True)
                else:
                    col1.write("—")
    ...
    if st.session_state.get('treino_ativo'):
        ...
        with st.form("form_treino"):
            ...
            for i, row in df_exercicios.iterrows():
                ...
                if row['imagem_url']: 
                    col_img.markdown(f'<a href="{row["imagem_url"]}" target="_blank"><img src="{row["imagem_url"]}" width="50" style="border-radius:8px;"></a>', unsafe_allow_html=True)
...
with aba_fichas:
    ...
    st.session_state.cor_zebrado = st.color_picker("Cor da linha alternada", st.session_state.cor_zebrado)
